...
export const mostrarReportes=(callback) => {
    ...
    'JOIN (SELECT DISTINCT MONTH(fecha_registro) as mes FROM orden WHERE MONTH(fecha_registro) = MONTH(CURDATE())) AS mes ' +
    ...
    'FROM (SELECT DISTINCT MONTH(fecha_registro) as mes FROM orden WHERE MONTH(fecha_registro) = MONTH(CURDATE())) AS mes ' +
